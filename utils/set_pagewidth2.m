...
position = get(gcf, 'Position');          % Width of the figure
tightinset = get(gcf, 'TightInset');      % Width of the surrounding text
total_width = position(3) + tightinset(3) + tightinset(1);
scale_factor = (pagewidth*fraction)/total_width;
set(gcf, 'Position', [position(1:2), position(3:4).*scale_factor]);
